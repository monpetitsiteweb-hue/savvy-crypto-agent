 ...
 *
 * One-time USDC approval to Permit2 contract on Base.
 * This enables BUY trades to skip Permit2 per-trade signing.
 * 
 * Supports two modes:
 * 1. User auth mode: Called with user JWT
 * 2. Operator mode: Called with service role + body.user_id
 */
...
function logStep(step: string, data: Record<string, unknown> = {}) {
  console.log("[approve-usdc-permit2]", JSON.stringify({ step, ...data }));
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse body for optional user_id (operator mode)
    ...
    // Determine user ID - from JWT or from body.user_id (operator mode)
    let userId: string;
    const authHeader = req.headers.get("Authorization") || req.headers.get("authorization");
    
    if (body.user_id) {
      // Operator mode: use provided user_id
      userId = body.user_id;
    ...
    const currentAllowance = await getCurrentAllowance(USDC_BASE, wallet.wallet_address, PERMIT2_ADDRESS);
    logStep("current_allowance", { allowance: currentAllowance.toString() });

    if (currentAllowance > BigInt("1000000000000")) {
      return jsonSuccess({
        status: "already_approved",
        allowance: currentAllowance.toString(),
        message: "USDC already approved to Permit2",
      });
    }
    ...
    const approveData = encodeApprove(PERMIT2_ADDRESS, MAX_UINT256);
    ...
    logStep("tx_build", { to: USDC_BASE, spender: PERMIT2_ADDRESS });
    ...
    return jsonSuccess({
      ...
      token: USDC_BASE,
      message: "USDC approval to Permit2 submitted. BUY trades will now skip Permit2 signing.",
    });
