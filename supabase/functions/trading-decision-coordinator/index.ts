...
serve(async (req) => {
  ...
  try {
    ...
    if (
      intent.execution_source === "manual" &&
      (mode === "mock" || intent.metadata?.force === true) &&
      intent.metadata?.executionMode === "mock"
    ) {
    ...
    if (intent.side === "BUY") {
      ...
      if (availableEur < tradeAllocation) {
        ...
        if (adjustedAllocation < 10) {
          // Minimum €10 trade
          console.log(
            `🚫 DIRECT: Insufficient balance - €${availableEur.toFixed(2)} available, €${tradeAllocation} requested (isMockMode=${isMockMode})`,
          );
          return {
            success: false,
