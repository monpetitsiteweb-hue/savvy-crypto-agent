...
async function computeFusedSignalScore(params: ComputeFusedSignalParams): Promise<FusedSignalResult> {
  ...
  try {
    ...
    console.log(`[SignalFusion] ${symbol}/${horizon}: ${rawCount} raw → ${dedupedSignals.length} after dedup`);

    // Fetch registry and strategy weights in parallel
    const [registryResult, weightsResult] = await Promise.all([
      supabaseClient.from("signal_registry").select("*").in("key", [...new Set(dedupedSignals.map((s: any) => s.signal_type))]),
  ...
  try {
    ...
    if (intent.side === 'BUY') {
      try {
        ...
        // Moved here (alongside A1) so it executes BEFORE any early return (fusion threshold, panic, etc.)
          ...
          if (SHADOW_5M_SYMBOLS.includes(pairSymbol)) {
            try {
              ...
              if (feat5mErr) {
                ...
                const ema50Threshold = ema50 * 0.995;
    ...
    if (execClass.target === "REAL") {
      ...
      //
      // ARCHITECTURE: Coordinator calls ONLY onchain-sign-and-send with raw params.
      ...
      if (intent.source === "manual" && hasWalletId) {
        ...
        // Determine amount based on side
        // BUY: eurAmount is EUR/USDC to spend
        ...
        try {
          ...
          if (!signSendResponse.ok) {
            const errorText = await signSendResponse.text();
            console.error("❌ COORDINATOR: onchain-sign-and-send failed:", errorText);
            throw new Error(`Execution failed: ${errorText}`);
          }
          ...
          if (!signSendData.ok || !signSendData.tx_hash) {
            console.error("❌ COORDINATOR: onchain-sign-and-send response invalid:", signSendData);
            throw new Error(signSendData.error?.message || signSendData.error?.code || "Execution returned invalid response");
          }
