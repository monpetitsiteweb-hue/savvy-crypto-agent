...
function jsonError(status: number, message: string, meta: Record<string, unknown> = {}) {
  ...
  return new Response(
    JSON.stringify({ success: false, error: message, ...meta }),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (wallet.chain_id !== 8453) {
      return jsonError(400, 'Invalid wallet chain (expected Base / 8453)', { step: 'wallet_lookup', chain_id: wallet.chain_id });
    }
...
async function getErc20Balance(tokenAddress: string, holder: string): Promise<bigint> {
  ...
  const response = await fetch(BASE_RPC, {
    ...
    body: JSON.stringify({
      ...
      method: 'eth_call',
      params: [
        { to: tokenAddress, data: dataField },
        'latest',
      ],
    }),
  });
