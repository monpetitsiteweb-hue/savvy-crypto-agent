-- Eliminate overload ambiguity for get_portfolio_metrics
DROP FUNCTION IF EXISTS public.get_portfolio_metrics(uuid, boolean);
DROP FUNCTION IF EXISTS public.get_portfolio_metrics(boolean, uuid);
...
-- PostgREST schema cache invalidation (re-ALTER after creation)
ALTER FUNCTION public.get_portfolio_metrics(uuid, boolean) SET search_path = public;
