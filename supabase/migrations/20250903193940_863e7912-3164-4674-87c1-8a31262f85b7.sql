-- Fix SELL execution: test mode bypass + symbol normalization + strategy scoping
...
DECLARE
  ...
  -- CRITICAL FIX: NO FEES IN TEST MODE
...
BEGIN
  ...
  -- CRITICAL FIX: Normalize symbol consistently FIRST - remove -EUR suffix for matching
  v_symbol := UPPER(TRIM(NEW.cryptocurrency));
  v_symbol := REPLACE(v_symbol, '-EUR', '');
  NEW.cryptocurrency := v_symbol;

  -- TEST MODE BYPASS: Skip coverage validation for test mode
  IF NEW.is_test_mode = true THEN
    -- Set basic snapshot fields for test mode
    NEW.original_purchase_amount := v_sell_amount;
    NEW.original_purchase_value := v_exit_value;
    NEW.original_purchase_price := v_exit_price;
    ...
    RAISE NOTICE 'TEST MODE: Bypassing coverage check for symbol % - amount %', 
      v_symbol, v_sell_amount;
    
    RETURN NEW;
  END IF;

  -- Debug logging
  RAISE NOTICE 'TRIGGER: Processing sell for symbol % (normalized from %) - strategy % - TEST MODE: % - FEE RATE: %', 
    v_symbol, UPPER(TRIM(NEW.cryptocurrency)), NEW.strategy_id, NEW.is_test_mode, v_fee_rate;
  ...
  FOR lot_record IN
    ...
    FROM available_buys
    WHERE remaining_amount > 1e-9  -- Use same tolerance as loop
    ORDER BY id
  LOOP
...
-- Create debug view for coverage tracking
CREATE OR REPLACE VIEW mock_coverage AS
SELECT 
  user_id,
  strategy_id,
  REPLACE(UPPER(cryptocurrency), '-EUR', '') as symbol,
  is_test_mode,
  SUM(CASE WHEN trade_type = 'buy' THEN amount ELSE 0 END) as total_bought,
  SUM(CASE WHEN trade_type = 'sell' THEN COALESCE(original_purchase_amount, amount) ELSE 0 END) as total_sold,
  SUM(CASE WHEN trade_type = 'buy' THEN amount ELSE 0 END) - 
  SUM(CASE WHEN trade_type = 'sell' THEN COALESCE(original_purchase_amount, amount) ELSE 0 END) as available
FROM public.mock_trades
WHERE user_id IS NOT NULL
GROUP BY user_id, strategy_id, REPLACE(UPPER(cryptocurrency), '-EUR', ''), is_test_mode
HAVING SUM(CASE WHEN trade_type = 'buy' THEN amount ELSE 0 END) > 0
   OR SUM(CASE WHEN trade_type = 'sell' THEN COALESCE(original_purchase_amount, amount) ELSE 0 END) > 0
ORDER BY user_id, strategy_id, symbol;
