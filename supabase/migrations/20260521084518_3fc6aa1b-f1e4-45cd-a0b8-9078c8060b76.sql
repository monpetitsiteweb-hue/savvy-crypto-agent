CREATE OR REPLACE FUNCTION public.get_portfolio_metrics(p_user_id uuid, p_is_test_mode boolean DEFAULT true)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN (
      ...
      WHERE user_id = p_user_id
        ...
        AND (p_is_test_mode = true OR execution_confirmed = true)
    ...
    WHERE b.user_id = p_user_id
      ...
      AND (p_is_test_mode = true OR b.execution_confirmed = true)
  ...
  -- PR B20 Fix 1: Total P&L is the accounting invariant: portfolio value minus
  -- starting capital. Gas and DEX fees are already reflected in cash_balance_eur
  -- (paid on-chain at swap time). Do NOT re-subtract them here — that would
  -- double-count. total_gas_eur and total_fees_eur remain exposed for
  -- informational display only.
  v_total_pnl := v_portfolio_value - v_starting;

  RETURN json_build_object(
    'success', true,
    'queried_mode', p_is_test_mode,
