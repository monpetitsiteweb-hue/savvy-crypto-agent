-- Convert table-returning SECURITY DEFINER functions to SECURITY INVOKER
-- These are being flagged as "views" by the linter

-- Convert admin_list_past_positions to use SECURITY INVOKER
-- This function can rely on RLS policies instead of SECURITY DEFINER
...
AS $function$
  -- Function will now rely on RLS policies for access control
  -- Only admins should be able to call this based on application logic
  SELECT
    id AS sell_trade_id,
...
-- Convert admin_seed_sequence to use SECURITY INVOKER  
-- This function should only be called by admins in controlled contexts
...
BEGIN
  -- Function now relies on RLS policies - caller must have appropriate permissions
