-- Fix the sell snapshot trigger to NEVER apply fees in test mode
...
BEGIN
  ...
  -- CRITICAL FIX: Normalize symbol consistently - remove -EUR suffix for matching
  ...
  -- Debug logging
  RAISE NOTICE 'TRIGGER: Processing sell for symbol % (normalized from %) - TEST MODE: % - FEE RATE: %', 
    v_symbol, UPPER(TRIM(NEW.cryptocurrency)), NEW.is_test_mode, v_fee_rate;

  -- FIFO: Match this SELL with previous BUY trades using normalized symbol
  FOR lot_record IN
