...
-- Step 4: Drop the existing user SELECT policy
...
-- Users can only view through the safe view or function
CREATE POLICY "Users can view safe connection data only"
ON public.user_coinbase_connections
FOR SELECT
TO authenticated
USING (false); -- Block direct SELECT access for users
...
-- (these operations don't expose the encrypted data in SELECT results)
...
CREATE OR REPLACE FUNCTION public.audit_connection_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Create trigger for audit logging (only for admin access)
CREATE TRIGGER audit_encrypted_credentials_access
  AFTER SELECT ON public.user_coinbase_connections
  FOR EACH ROW
  WHEN (has_role(auth.uid(), 'admin'::app_role))
  EXECUTE FUNCTION public.audit_connection_access();