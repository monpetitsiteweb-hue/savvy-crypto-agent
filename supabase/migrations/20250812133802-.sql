...
-- BEFORE INSERT trigger function: compute FIFO snapshot for SELLs
...
DECLARE
  ...
  -- cursor over prior BUY liquidity in FIFO order
  CURSOR_BUYS CURSOR FOR
    WITH buys AS (
...
BEGIN
  ...
  -- walk FIFO buys
  FOR lot_amt, lot_price IN
    SELECT remaining, price FROM CURSOR_BUYS
  LOOP
    EXIT WHEN need <= 0;
    take := LEAST(need, ROUND(lot_amt::numeric, 8));
    IF take > 0 THEN
      acc_amt := ROUND((acc_amt + take)::numeric, 8);
      acc_val := ROUND((acc_val + (take * ROUND(lot_price::numeric,2)))::numeric, 2);
...
-- trigger attaches to table; fires only for SELLs
DROP TRIGGER IF EXISTS mt_sell_bi_trg ON public.mock_trades;
CREATE TRIGGER mt_sell_bi_trg
BEFORE INSERT ON public.mock_trades
FOR EACH ROW
WHEN (NEW.trade_type = 'sell')
