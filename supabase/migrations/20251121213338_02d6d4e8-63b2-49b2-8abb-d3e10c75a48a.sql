-- Update mt_on_sell_snapshot trigger to trust manual snapshot when original_trade_id is present
-- For targeted manual SELLs, skip FIFO and use pre-filled snapshot from coordinator
...
DECLARE
  ...
  -- Targeted manual SELL detection
...
BEGIN
  ...
  -- Detect targeted manual SELL with pre-filled snapshot
  v_has_pre_filled_snapshot := (
    v_is_manual_override 
    AND NEW.original_trade_id IS NOT NULL 
    AND NEW.original_purchase_amount IS NOT NULL
    AND NEW.original_purchase_value IS NOT NULL
  );

  -- TARGETED MANUAL SELL: Trust pre-filled snapshot from coordinator
  IF v_has_pre_filled_snapshot THEN
    -- Use the pre-filled snapshot directly (no FIFO computation)
    total_purchase_amount := NEW.original_purchase_amount;
    total_purchase_value := NEW.original_purchase_value;
    
    -- Ensure exit_value is consistent
    NEW.exit_value := v_exit_value;
    
    -- Skip FIFO loop entirely (no need to match lots)
    need_amount := 0;
    
    -- Go directly to fee + PnL computation
    RAISE NOTICE 'TARGETED MANUAL SELL: Using pre-filled snapshot for original_trade_id=%, amount=%, value=%', 
      NEW.original_trade_id, total_purchase_amount, total_purchase_value;
  ELSE
    -- STANDARD FIFO LOGIC: Match this SELL with previous BUY trades
    FOR lot_record IN
    ...
    -- Verify we have enough BUY coverage (bypass for manual overrides without snapshot)
    ...
    -- Set snapshot fields from FIFO results
