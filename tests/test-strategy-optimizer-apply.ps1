# Test Strategy Optimizer - Apply Action
# ⚠️ CAUTION: This writes validated parameter adjustments to the database

param(
    [string]$ProjectId = "fuieplftlcxdfkxyqzlt",
    [string]$AnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ1aWVwbGZ0bGN4ZGZreHlxemx0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTIyMjg3OTQsImV4cCI6MjA2NzgwNDc5NH0.t1DwSViIf_ya-7fUTqM5d56CPINq0JdAYt-YFJs8fa8",
    [string]$UserJWT = "eyJhbGciOiJIUzI1NiIsImtpZCI6IkdkZmlpck5OS2hTSlc0ZnYiLCJ0eXAiOiJKV1QifQ..aHeJu_tOrwmebdkPVF71GSpaDdNXjCFtaUpqtWcQEIU",
    [string]$StrategyId = "5f0664fd-98cb-4ec2-8c2b-95cb1a28b80e"
)

$ErrorActionPreference = "Stop"

Write-Host "⚠️  WARNING: This action will write parameter adjustments to the database!" -ForegroundColor Red
Write-Host "Press Ctrl+C to cancel, or any other key to continue..." -ForegroundColor Yellow
$null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
Write-Host ""

$url = "https://$ProjectId.supabase.co/functions/v1/strategy-optimizer"

$headers = @{
    "Authorization" = "Bearer $UserJWT"
    "apikey" = $AnonKey
    "Content-Type" = "application/json"
}

$body = @{
    action = "apply"
    strategyId = $StrategyId
} | ConvertTo-Json

Write-Host "💾 Testing strategy-optimizer APPLY action..." -ForegroundColor Cyan
Write-Host "URL: $url" -ForegroundColor Gray
Write-Host "Strategy ID: $StrategyId" -ForegroundColor Gray
Write-Host ""

try {
    $response = Invoke-RestMethod -Uri $url -Method Post -Headers $headers -Body $body
    
    Write-Host "✅ Success!" -ForegroundColor Green
    Write-Host ""
    Write-Host "Applied Parameter Changes:" -ForegroundColor Yellow
    $response | ConvertTo-Json -Depth 10
    
} catch {
    Write-Host "❌ Error:" -ForegroundColor Red
    Write-Host $_.Exception.Message
    if ($_.ErrorDetails.Message) {
        Write-Host "Details:" -ForegroundColor Yellow
        $_.ErrorDetails.Message
    }
    exit 1
}
