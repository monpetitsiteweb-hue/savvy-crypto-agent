...
|-----|------------------|-----------------|
| `settle_buy_trade` | Writes to user's row | Add `p_is_test_mode` param, filter by mode |
| `settle_sell_trade` | Writes to user's row | Add `p_is_test_mode` param, filter by mode |
| `reset_portfolio_capital` | Deletes/resets user's row | Add `p_is_test_mode` param, filter by mode |
| `reserve_capital` | Updates reserved_eur | Add `p_is_test_mode` param, filter by mode |
| `release_reservation` | Updates reserved_eur | Add `p_is_test_mode` param, filter by mode |
...
| RPC signature change breaks frontend | Medium | High | Update all callers in same deployment |
...
| `SELECT COUNT(*) FROM portfolio_capital WHERE is_test_mode = false;` | 0 (no REAL rows yet) |
